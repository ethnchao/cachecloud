...
public class RedisProtocol {
    ...
    private static final String RUN_SHELL = "%s/redis-server %s > " + MachineProtocol.LOG_DIR + "redis-%d-%s.log 2>&1 &";

    private static final String SENTINEL_SHELL = "%s/redis-server %s --sentinel > " + MachineProtocol.LOG_DIR + "redis-sentinel-%d-%s.log 2>&1 &";
    ...
    public static String getRunShell(int port, boolean isCluster) {
        return String.format(RUN_SHELL, ConstUtils.CACHECLOUD_REDIS_BASE_DIR, MachineProtocol.CONF_DIR + getConfig(port, isCluster), port, DateUtil.formatYYYYMMddHHMM(new Date()));
    }

    public static String getSentinelShell(int port) {
        return String.format(SENTINEL_SHELL, ConstUtils.CACHECLOUD_REDIS_BASE_DIR, MachineProtocol.CONF_DIR + getConfig(port, false), port, DateUtil.formatYYYYMMddHHMM(new Date()));
    }
    ...
    public static String getExecuteCommandShell(String host, int port, String password, String command) {
    		...
    		shell.append(String.format("%s/redis-cli -h %s -p %s", ConstUtils.CACHECLOUD_REDIS_BASE_DIR, host, port));
